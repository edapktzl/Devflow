...
async function start(){if(!token)return;me=await api('/me');$('#identity').textContent='@'+me.name;$('#auth').hidden=true;$('#workspace').hidden=false;const query=new URLSearchParams(location.search);const integration=query.get('integration');if(integration&&query.get('status')==='connected'){notice(`${integration==='slack'?'Slack':'GitHub'} bağlantısı başarıyla tamamlandı.`);history.replaceState({},'',location.pathname);}const orgs=await api('/organizations');orgRoles=new Map(orgs.map(item=>[item.id,item.role]));options($('#org'),orgs);const callbackOrg=query.get('org'),rememberedOrg=callbackOrg||sessionStorage.getItem('devflow-org');org=orgs.some(item=>item.id===rememberedOrg)?rememberedOrg:(orgs[0]?.id||'');$('#org').value=org;if(org)sessionStorage.setItem('devflow-org',org);const deep=query.get('task');if(deep){const t=await api(`/tasks/${Number(deep)}`);for(const o of orgs){const projects=await api(`/organizations/${o.id}/projects`);if(projects.some(p=>p.id===t.project_id)){org=o.id;$('#org').value=org;sessionStorage.setItem('devflow-org',org);preferredPid=t.project_id;break;}}}await loadOrg();}
async function loadOrg(){streamController?.abort();if(!org){sessionStorage.removeItem('devflow-project');$('#kanban').innerHTML='<p class="empty">İlk organizasyonunuzu oluşturun.</p>';return;}members=await api(`/organizations/${org}/members`);const projects=await api(`/organizations/${org}/projects`);options($('#project'),projects);const rememberedPid=preferredPid||sessionStorage.getItem('devflow-project');if(rememberedPid&&projects.some(item=>item.id===rememberedPid))$('#project').value=rememberedPid;preferredPid=null;pid=$('#project').value;if(pid)sessionStorage.setItem('devflow-project',pid);else sessionStorage.removeItem('devflow-project');await loadProject();}
...
$('#org').onchange=safe(async()=>{clearWorkspaceState();org=$('#org').value;if(org)sessionStorage.setItem('devflow-org',org);await loadOrg();});$('#project').onchange=safe(async()=>{pid=$('#project').value;if(pid)sessionStorage.setItem('devflow-project',pid);await loadProject();});
$('#newOrg').onclick=safe(async()=>{const name=prompt('Organizasyon adı');if(!name)return;const created=await api('/organizations','POST',{name});if(created?.id)sessionStorage.setItem('devflow-org',created.id);await start();});
$('#newProject').onclick=safe(async()=>{if(!org)throw Error('Önce organizasyon oluşturun');const name=prompt('Proje adı');if(!name)return;const created=await api(`/organizations/${org}/projects`,'POST',{name});if(created?.id){preferredPid=created.id;sessionStorage.setItem('devflow-project',created.id);}await loadOrg();});
...
for(const provider of ['github','slack'])$('#'+provider+'Connect').onclick=safe(async()=>{if(!org)throw Error('Önce bir organizasyon oluşturun.');sessionStorage.setItem('devflow-org',org);if(pid)sessionStorage.setItem('devflow-project',pid);const r=await api(`/organizations/${org}/oauth/${provider}`,'POST',{});location.href=r.url;});
...
safe(start)().finally(()=>document.body.classList.remove('app-loading'));
