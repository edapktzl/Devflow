...
CREATE TABLE IF NOT EXISTS members(org_id TEXT REFERENCES organizations(id),user_id TEXT REFERENCES users(id),role TEXT CHECK(role IN ('Owner','Admin','Member','Viewer')),PRIMARY KEY(org_id,user_id));
...
CREATE TABLE IF NOT EXISTS idempotency(user_id TEXT,key TEXT,request_hash TEXT,response TEXT,PRIMARY KEY(user_id,key));
