...
export async function tick(){
 ...
 const heartbeat=setInterval(()=>run("UPDATE jobs SET lease_until=? WHERE id=? AND status='running'",now()+60000,job.id),20000);
 try{const data=JSON.parse(job.payload);if(['github','event'].includes(job.kind))tx(()=>{if(job.kind==='github')githubDelivery(data);else processEvent(data.id);run("UPDATE jobs SET status='done',lease_until=NULL,error=NULL WHERE id=?",job.id);});else{if(job.kind==='slack')await sendSlack(data);else if(job.kind==='resync')await resync(data.project_id);else throw Error('Unknown job kind');run("UPDATE jobs SET status='done',lease_until=NULL,error=NULL WHERE id=?",job.id);}}
